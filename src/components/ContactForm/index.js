import { IconBox, IconChevronDown, IconEmail, IconHandMoney, IconHomeDesign, IconLoading, IconPhone, IconUser } from "../Icons";
...
export default function ContactForm({ onClose, className }) {
  ...
  const onSubmit = async (value) => {
    ...
    validateField("design_style", /^.{1,}$/, "Vui lòng chọn phong cách thiết kế.");
    validateField("investment_level", /^.{1,}$/, "Vui lòng chọn mức độ đầu tư.");

    const totalArea = parseFloat(value.total_area);
    if (isNaN(totalArea) || totalArea <= 0) {
      newErrorMessages.total_area = "Tổng diện tích phải là một số dương.";
    }

    validateField("phone", /^[0-9]{10,}$/, "Số điện thoại không hợp lệ.");
    validateField("email", /^[^\s@]+@[^\s@]+\.[^\s@]+$/, "Địa chỉ email không hợp lệ.");
  ...
  return (
    <form style={{ width: "100%", height: "100%" }} onSubmit={handleSubmit(onSubmit)} className={clsx("flex flex-col space-y-4", className || "")}>
      <div className="flex">
      ...
      {errorMessages.design_style && <div className="text-sm text-left text-red-500">{errorMessages.design_style}</div>}
      <div className="flex">
        <div className="flex items-center justify-center border border-gray-400 hover:border-gray-500 rounded-l-lg border-r-0 w-12">
          <IconHandMoney />
        </div>
      ...
      {errorMessages.investment_level && <div className="text-sm text-left text-red-500">{errorMessages.investment_level}</div>}
      <div className="flex">
        <div className="flex items-center justify-center border border-gray-400 hover:border-gray-500 rounded-l-lg border-r-0 w-12">
          <IconBox />
        </div>
      ...
      {errorMessages.total_area && <div className="text-sm text-left text-red-500">{errorMessages.total_area}</div>}
      <div className="flex">
        <div className="flex items-center justify-center border border-gray-400 hover:border-gray-500 rounded-l-lg border-r-0 w-12">
          <IconUser />
        </div>
      ...
      {errorMessages.full_name && <div className="text-sm text-left text-red-500">{errorMessages.full_name}</div>}
      <div className="flex">
        <div className="flex items-center justify-center border border-gray-400 hover:border-gray-500 rounded-l-lg border-r-0 w-12">
          <IconPhone />
        </div>
      ...
      {errorMessages.phone && <div className="text-sm text-left text-red-500">{errorMessages.phone}</div>}
      <div className="flex">
        <div className="flex items-center justify-center border border-gray-400 hover:border-gray-500 rounded-l-lg border-r-0 w-12">
          <IconEmail />
        </div>
      ...
      {errorMessages.email && <div className="text-sm text-left text-red-500">{errorMessages.email}</div>}
      ...
      >
        ...
        <div
          className={clsx("opacity-0 transition-opacity absolute top-1/2 -translate-y-1/2 right-2", {
